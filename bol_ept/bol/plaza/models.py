...
class Model(object):
    ...
    def parse(cls, api, xml):
        ...
        for element in xml.getchildren():
            tag = element.tag.partition('}')[2].replace("-", "_")
...
class OrderItem(Model):
    ...
    class Meta:
        OrderItemId = TextField()
        OrderId = TextField()
        OrderDate = DateTimeField()
        PromisedDeliveryDate=DateTimeField()
        EAN=TextField()
        Title=TextField()
        OfferPrice = DecimalField()
        TransactionFee = DecimalField()
        Quantity = IntegerField()
        OfferCondition=TextField()
        OfferReference=TextField()
        FulfilmentMethod=TextField()
...
class Item(Model):
    ...
    class Meta:
        ...
        ReturnReasonComments = TextField()
        CustomerDetails = ModelField(CustomerDetails)
...
class FbbTransport(Model):
    
    class Meta:
        Name=TextField()
        Code=TextField()

class FbbTransports(ModelList):
    
    class Meta:
        item_type=FbbTransport()
...
class DeliveryWindow(ModelList):
...
class StateTransition(Model):
    ...
    class Meta:
        InboundState=ModelField(InboundState)
...
class ServiceErrors(ModelList):
    
    class Meta:
        item_type=ServiceError()

class Errors(ModelList):
    
    class Meta:
        item_type=ServiceErrors()
